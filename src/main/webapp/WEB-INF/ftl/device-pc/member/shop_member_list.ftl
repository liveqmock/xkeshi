...
<div class="rwrap">
	<div class="r_title"><span class="fl">会员列表</span>
	   <div class="search_wrap">
			<form action="${base}/member/shop/list" class="search_form fl">
			...
			<a href="${base}/member/shop/add" class="new_a fl ml12"></a>
		</div>
		<#if (searcher != null && searcher.hasParameter)! >
			<div class="search_result ofw">
				<p class="result_tit"><span class="result_tit_text">搜索/筛选</span>
					<#if searcher.key><span class="b_tit b_tit1" data-fid="1">名称/昵称：${(searcher.key)!}<b>x</b></span></#if>
					<#if searcher.mobile><span class="b_tit b_tit1" data-fid="6">手机号：${(searcher.mobile)!}<b>x</b></span></#if>
					<#if (searcher.memberType.id)??><span class="b_tit b_tit1" data-fid="2">会员类型:
					...
					<#if searcher.gender><span class="b_tit b_tit1" data-fid="3">性别:<#if (searcher.gender=="male")!>男<#else>女</#if><b>x</b></span></#if>
					<#if searcher.createStartDate><span class="b_tit ml85 b_tit1" data-fid="4">起始创建时间：${(searcher.createStartDate?string('yyyy-MM-dd HH:mm:ss'))!}<b>x</b></span></#if>
					<#if searcher.createEndDate><span class="b_tit b_tit1" data-fid="5">截止创建时间：${(searcher.createEndDate?string('yyyy-MM-dd HH:mm:ss'))!}<b>x</b></span></#if>
					</span><span class="ml12">的结果</span></span>
				</p>
				<a href="${(base)!}/member/shop/list" class="s_clear">清空</a>
			</div>
		</#if>
		</div>
		<table class="tb_main">
			<#if (!pager.list?? || pager.list?size==0) && ( searcher == null || !searcher.hasParameter)>
				<div class="new_hint_wrap">
					...
					您还没有会员，您可以通过收银台客户端会员管理菜单进行会员添加，也可以点击右上角
					<img src="${static_base}/css/img/admin_frame/new_a.png">按钮添加会员
				</div>
			<#else>
				<tr class="th">
			...
			<tr <#if member_index%2==0>class="tr_bg"</#if>>
				...
				<td class="merc_name">
					<a href="${base}/member/${(member.eid)!}" class="b_a"><#if (member.name)?length == 0 >(空)<#else>${(member.name)!'(空)'}</#if></a>
				</td>
		...
		<div class="pb pop_filter">
			<form action="${base}/member/shop/list" method="get" class="f_form">
				<input type="hidden" name="key" value="<#if (searcher.key)?? >${searcher.key}</#if>" class="f_input fi_1"/>
				<div class="pb_title">筛选</div>
				<div class="pb_main" style="padding:0;">
					<div class="pb_item">
						<p class="pb_item_title">会员类型:</p>
						<select name="memberType.id" style="width: 213px;color:#000000;" class="f_input pb_item_input select_style fi_2"> 
							<option value="" class="" >-会员类型-</option>
							<#list memberTypes as memberType>
					...
					<div class="pb_item">
						...
						<select name="gender" style="width: 213px;color:#000000;" class="f_input pb_item_input select_style fi_3">
