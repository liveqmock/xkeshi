...
<@m.page_header title="会员类型" selected='member' subselected='type' css='seller_list_new|member_type_info'  js="merchant_member_type_info"  />
<script>
...
<div class="rwrap">
	<div class="r_title"><span class="fl">会员类型管理</span>
		<div class="search_wrap">
			<form action="${base}/member/merchant/type/list" class="search_form f_form fl">
		...
		<#if searcher != null && searcher.hasParameter>
			<div class="search_result">
				<p class="result_tit"><span class="result_tit_text">搜索/筛选</span>
					<#if searcher.key><span class="b_tit" data-fid="1">${(searcher.key)!}<a href="${(base)!}/member/merchant/type/list"><b>x</b></a></span></#if>
					</span>的结果</span>
				</p>
				<a href="${(base)!}/member/merchant/type/list" class="s_clear">清空</a>
			</div>
		</#if>
	...
	<table class="tb_main">
		<#if (!pager.list?? || pager.list?size==0) && ( searcher == null || !searcher.hasParameter)>
			<div class="new_hint_wrap"><div class="new_hint_bg"></div>您还没有创建会员模板，点击右上角<img src="${static_base}/css/img/admin_frame/new_a.png">按钮创建新的会员模板吧</div>
		<#else>
			<tr class="th">
				<td class="" style="width:50px;">编号</td>
				<td class="" style="width:130px;">名称</td>
				<td class="" style="width:90px;">折扣</td>
				<td class="" style="width:90px;">默认会员类型</td>
				<td class="" style="width:100px;">创建时间</td>
				<td class="" style="width:100px;">修改时间</td>
				<td class="" style="width:80px;">操作</td>
			</tr>
		</#if>
		<#list pager.list as memberType>
			<tr <#if memberType_index%2==0>class="tr_bg"</#if>>
				<td class="">${memberType_index+1}</td>
				<td class="">${(memberType.name)!}</td>
				<td class="">
				...
				<td class=""><#if (memberType.default)!>是<#else>否</#if></td>
				<td class="">${(memberType.createdTime?string('YY-MM-dd HH:mm'))!'-'}</td>
				<td class="">${(memberType.updatedTime?string('YY-MM-dd HH:mm'))!'-'}</td>
				<td class="" style="white-space: nowrap;">
					...
					<#if !memberType.default>
						<a href="javascript:void(0);" class="b_a pop_a del_a" style="width:30px;"  data-pop="pop_del" data-para="/member/merchant/type/delete/${(memberType.id)!}" data-name="${(memberType.name)!}"
							data-defaulted = "${(memberType.default?string('true','false'))!}"
	...
	<div class="pb add_opt">
		<form action="${base}/member/merchant/type/add" method="POST" id="typeForm" name="typeForm" enctype="multipart/form-data">
			...
			<div class="pb_main">
				<div class="pb_item  pb_nameId">
					<p class="pb_item_title">名称</p>
					<input type="text" class="pb_item_input lh14" name="name" id="nameId" placeholder="会员类型名称">
				</div>
				...
				<div class="pb_item pb_card">
					...
					<a href="" target="_blank"  class="pb_input show_cover_picture" style="margin-right:10px;">点击查看会员卡</a>建议尺寸:555×330
				</div>
			</div>
	...
	<div class="pb pop_del">
		<form action="" method="POST">
			...
			<div class="pb_main">
				<p class="pb_item_title" style="padding:14px 0;">确认删除:<span class="pb_name"></span></p>
			</div>
