...
<@m.page_header title="上传图片"  selected='shop' subselected='album' css="admin_frame|seller_info|add_photo"/>
<script>
	var pic = '${pic}';
	$(function(){
		...
		$('.add_photo_btn').click(function(){
			...
			if(pic!='' && pic!=null) {
			  jQuery("form").attr("action","${base}/album/edit/${(pic.id)!}").submit();
			}else {
				if(""==file) {
					alert("请先上传文件再保存！");
					return false;
				}
			  jQuery("form").attr("action","${base}/album").submit();
			}
		})
	})
</script>
...
<div class="rwrap">
	<form method="POST" enctype="multipart/form-data">
		<p class="r_title"><a class="back_a" href="${base}/album/list"></a><#if pic??>修改图片<#else>上传图片</#if></p>
		<div class="tb_wrap">
			<table class="tb_main">
				<tr>
		...
		<div class="btn_wrap"><button type="button" class="green_btn_l add_photo_btn">保存</button><span class="btn_split">或</span><a href="${base}/album/list" class="cancel_a">取消</a></div>
	</form>
