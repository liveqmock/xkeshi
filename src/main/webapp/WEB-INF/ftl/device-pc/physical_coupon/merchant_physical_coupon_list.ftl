...
<@m.page_header selected='physical_coupon' subselected='list' title='集团实体券管理' js='member_type_info|list_filter' css='seller_list_new|set_mime|voucher_manage'/>
<div class="rwrap">
	<div class="r_title"><span class="fl">集团实体券管理</span>
	<div class="search_wrap ">
	<form class="search_form f_form fl" action="${base}/physical_coupon/merchant/list">
			<input type="text" name="name" <#if (searcher.name)?? >value="${searcher.name}"</#if> placeholder="实体券名称" class="search_input" >
		...
		<#if searcher != null && searcher.hasParameter >
			<div class="search_result">
				<p class="result_tit"><span class="result_tit_text">搜索/筛选</span>
					...
					<#if searcher.businessType>
						 <span class="b_tit" data-fid="3">来源商户:
						 <#if (searcher.businessType)!  == 'MERCHANT' >集团</#if>
						 <#if (searcher.businessType)!  == 'SHOP' >
							<#list shopResourceList as shop>
								<#if  (searcher.businessId)! == shop.id >${shop.name}</#if>
							</#list>
						</#if>
						<b>x</b></span>
					</#if>
					<#if searcher.shopIds>
						<span class="b_tit" data-fid="4">适用商户:
							<#list shopResourceList as shop>
				...
				<a href="${base}/physical_coupon/merchant/list" class="s_clear">清空</a>
			</div>
		</#if>
	</div>	
	<table class="tb_main">
		...
		<#else>
			...
			<#list pager.list as physicalCoupon>
				<tr <#if physicalCoupon_index%2==0>class="tr_bg"</#if>>
					...
					<td class="suit">${(physicalCoupon.shopCount)!}</td>
					<td class="status"><#if (physicalCoupon.enable)! == true>启动<#else>暂停</#if></td>
					<td class="from">${(physicalCoupon.shopName)!}</td>
					<td class="operate"><a href="/physical_coupon/${physicalCoupon.id}/detail" class="b_a">查看</a>
	...
	<div class="pb pop_filter">
			<form action="${base}/physical_coupon/merchant/list"  method="get" class="f_form">
			...
			<div class="pb_main">
				...
				<div class="pb_item">
					<p class="pb_item_title" name="suit">适用商户</p>
					<select class="pb_item_i pb_sel fi_4" name="shopIds">
						<option value="">请选择...</option>
						<#if shopResourceList>
							<#list shopResourceList as shop>
