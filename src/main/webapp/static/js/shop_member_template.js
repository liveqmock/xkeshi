...
$(function(){
	...
	$('.new_a').click(function(){
		isAddOpt =  true;
		document.attrPostForm.action= BASE+"/member/shop/attribute/save"; 
	...
	$('.pb_add_form_btn').click(function(){ 
		...
		if ($("#templateAttributeName").val().trim().length > 250) {
			alert("名称不能超过250个字符");
			return false;
		}
			...
			if ($('#productAttributeType').val()=='checkbox'||$('#productAttributeType').val()=='select'){
				...
				$("input[name='optionalValues']").each(function(i,v){
						...
						if(this.value.trim() == ''){
							...
							if (i+1 == $("input[name='optionalValues']").length) {
								$('#attrPostForm').submit();
							}
						}
				})
	...
	$('#productAttributeType').change(function(){
		if ($('.icon-del').size() == 0){
			$('.option-add').parent().before('<p class="input_item"  id="p_select_wrap"><input type="text" class="input-m pb_input" name="optionalValues" value="" id="optionalValues"><a href="javascript:" class="b_a icon-del">删除</a></p>');
			$('.icon-del').first().hide();
			$('.option-add').show();
		} 
	...
	$('.option-add').click(function(){
		if ($('.icon-del').size()>0){
			$('.icon-del').first().show();
			$(this).parent().before('<p class="input_item"  id="p_select_wrap"><input type="text" class="input-m pb_input" name="optionalValues" value="" id="optionalValues"><a href="javascript:" class="b_a icon-del">删除</a></p>');
		} 
	});
...
var updateAttirbute  = function(ts,id,name,attributeType,required,enabled,sequence){
	isAddOpt =  false;
	document.attrPostForm.action= BASE+"/member/shop/attribute/update";
	showAttributeOptionStore($(ts).data("optionalvalues"));
 ...
 * 注入会员属性下拉的值
 */
var showAttributeOptionStore = function (optionalValues){
	$("p").remove("#p_select_wrap");
	if (optionalValues.length > 0) {
/*		$.each(optionalValues, function(i,v){
			if(replaceVal(v).trim() != ''){
				  $('.option-add').parent().before('<p class="input_item"  id="p_select_wrap"><input type="text" class="input-m pb_input" name="optionalValues" value="'+v
						  +'" id="optionalValues"><a href="javascript:" class="b_a icon-del">删除</a></p>');
			}
		})*/
	}
	$('.pb_select_wrap').show();
...
var deleteAttribute  =  function(id ,name ,self){
	...
	document.memberTemplateForm.action= BASE+"/member/shop/attribute/delete/"+id;
